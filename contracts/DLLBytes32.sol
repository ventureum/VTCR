...
library DLLBytes32 {
	...
	function getNext(Data storage self, bytes32 curr) returns (bytes32) {
		return self.dll[curr].next;
	}

	function getPrev(Data storage self, bytes32 curr) returns (bytes32) {
		return self.dll[curr].prev;
	}

	function insert(Data storage self, bytes32 prev, bytes32 curr, bytes32 next) {
	...
	function remove(Data storage self, bytes32 curr) {
